...
1. **辞書を取得 — ショートカットの入力から**
2. **辞書の値を取得**: キー `parentID`
3. **辞書の値を取得**: キー `title`
4. **辞書の値を取得**: キー `listName`
5. **リマインダーを検索**: フィルタ「ID が _parentID_ と一致」 / 取得件数 1
6. **新規リマインダー**: タイトル _title_、リスト _listName_、**親項目** に 5 の結果を指定
